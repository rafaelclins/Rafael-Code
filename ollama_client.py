...
def _chamar_ollama_local(mensagens: list[dict], temperatura: float) -> str:
    payload = {
        "model": OLLAMA_MODEL,
        "messages": mensagens,
        "stream": False,
    ...
    if VERBOSE_MODE:
        logger.info(
            "--- PAYLOAD OLLAMA LOCAL ---\n%s",
            json.dumps(payload, indent=2, ensure_ascii=False),
        )
    resp = requests.post(OLLAMA_API_URL, json=payload, timeout=(30, 600))
    resp.raise_for_status()
    data = resp.json()
    if not isinstance(data, dict):
        raise ValueError(f"Resposta Ollama não é dict: {type(data).__name__}")
    texto = (data.get("message") or {}).get("content", "")
    if VERBOSE_MODE:
        logger.info("--- RESPOSTA OLLAMA LOCAL ---\n%s", texto)
    return texto


def _tentar_fallback_local(
    mensagens: list[dict],
    temperatura: float,
    ...
    if resposta_estava_vazia:
        ...
        logger.warning(
            "API Zen falhou após as tentativas. Executando fallback para modelo local..."
        )
    texto = _chamar_ollama_local(mensagens, temperatura)
    if not texto.strip():
...
def chamar_conversa(
    ...
    try:
        return _tentar_fallback_local(mensagens, temperatura, resposta_vazia)
    except ErroModelo:
        raise
    except Exception as e:
        raise ErroModelo(
            f"Falha após {MAX_TENTATIVAS_MODELO} tentativas no chat. "
            f"Fallback para modelo local ({OLLAMA_MODEL}) falhou: {e}",
...
def chamar_agente(
    ...
    try:
        texto_fallback = _tentar_fallback_local(
            mensagens, temperatura, resposta_vazia
        )
        dados_validados = extrair_e_validar_json(texto_fallback)
        esquema_pydantic(**dados_validados)
        return dados_validados
    except ErroModelo:
        raise
